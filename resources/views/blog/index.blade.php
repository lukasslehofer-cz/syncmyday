...
<div class="help-content">
    ...
    <p class="text-lg sm:text-xl text-gray-600 mb-6 sm:mb-8">{{ __('messages.blog_subtitle') }}</p>
            ...
            <a href="{{ route('blog.show', $article->slug) }}" class="group bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-xl overflow-hidden hover:shadow-lg transition">
                ...
                <div class="p-4 sm:p-6">
                    <div class="flex items-center mb-2 sm:mb-3">
                        <span class="inline-block px-2 sm:px-3 py-1 text-xs font-semibold text-indigo-600 bg-indigo-100 rounded-full">
                            {{ $article->category->getName() }}
                        </span>
                        @if($article->published_at)
                        <span class="ml-auto text-xs sm:text-sm text-gray-500">
    ...
    @else
        <div class="p-6 sm:p-8 bg-gray-50 border border-gray-200 rounded-xl text-center">
            <p class="text-sm sm:text-base text-gray-600">{{ __('messages.blog_no_articles') }}</p>
        </div>
    @endif
</div>
@endsection
