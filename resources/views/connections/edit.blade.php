                ...
                @else
                    ...
                    <div class="space-y-3">
                        @foreach($connection->available_calendars as $index => $calendar)
                            <label class="flex items-center p-4 border-2 border-gray-200 rounded-xl hover:border-indigo-300 hover:bg-indigo-50 transition cursor-pointer group {{ (old('selected_calendar_id', $selectedCalendarId) === $calendar['id']) ? 'border-indigo-400 bg-indigo-50' : '' }}">
                                <input 
                                    type="radio" 
