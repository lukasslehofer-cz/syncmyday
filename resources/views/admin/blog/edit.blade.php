...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <form action="{{ route('admin.blog.update', $article->id) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Slug (URL) {{ $locale === 'cs' ? '*' : '' }}</label>
                    <input type="text" name="slug_{{ $locale }}" value="{{ $translation?->slug }}" {{ $locale === 'cs' ? 'required' : '' }} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="muj-clanek">
